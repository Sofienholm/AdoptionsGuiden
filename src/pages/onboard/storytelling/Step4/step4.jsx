...
export default function Step4() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set([del3TextRef.current, del3DyrRef.current, del3HjemRef.current, del3ArbejdeRef.current],
        { opacity: 0, y: "120%" });
      ...
      // Del 1 ud
      tl.to(del1Ref.current, { opacity: 0, duration: 0.8, ease: "power2.out" });
      ...
      // Tabs ud igen
      tl.to(tabs,
        { y: "-120%", opacity: 0, duration: 1.0, ease: "power3.in", stagger: 0.15 });

      tl.to({}, { duration: 0.4 });

      // Del 2 ind
      tl.to(del2Ref.current, { opacity: 1, duration: 1.0, ease: "power2.out" });

      tl.to({}, { duration: 0.5 });

      // Del 2 ud
      tl.to(del2Ref.current, { opacity: 0, duration: 0.7, ease: "power2.inOut" });
      ...
      const del3Icons = [del3HjemRef.current, del3DyrRef.current, del3ArbejdeRef.current];

      tl.to(del3Icons,
        { y: "0%", opacity: 1, duration: 0.9, ease: "power3.out", stagger: 0.25 });
  ...
  // Data til popup-kort
  const cardConfig = {
    hyrde: { img: hyrdeKort, alt: "Hyrdehunde", link: "https://www.dyrenesbeskyttelse.dk/familiedyr/pasningsvejledninger/hund/hyrde-og-vagthunde" },
    jagt: { img: jagtKort, alt: "Jagthunde", link: "https://www.dyrenesbeskyttelse.dk/familiedyr/pasningsvejledninger/hund/apporterende-jagthunde" },
    selskab: { img: selskabKort, alt: "Selskabshunde", link: "https://www.dyrenesbeskyttelse.dk/familiedyr/pasningsvejledninger/hund/selskabshunde" },
    arbejd: { img: arbejdKort, alt: "Arbejdshunde", link: "https://www.dyrenesbeskyttelse.dk/familiedyr/pasningsvejledninger/hund/hyrde-og-vagthunde" },
    blanding: { img: blandingKort, alt: "Blandingshunde", link: "https://www.dyrenesbeskyttelse.dk/familiedyr/pasningsvejledninger/hund/blandinger" },
    mynde: { img: myndeKort, alt: "Mynder", link: "https://www.dyrenesbeskyttelse.dk/familiedyr/pasningsvejledninger/hund/mynder" },
  };
  ...
  useEffect(() => {
    ...
    gsap.timeline()
      ...
      .to(popupInnerRef.current, { opacity: 1, scale: 1, y: 0, duration: 0.4 }, "<0.05");
  }, [activeCard]);

  const handleClosePopup = () => {
    gsap.timeline({ onComplete: () => setActiveCard(null) })
      .to(popupInnerRef.current, { opacity: 0, scale: 0.9, y: 20, duration: 0.25 })
      ...
      {activeCard && currentConfig && (
        <div ref={popupOverlayRef} className={styles.popupOverlay}>
          <div ref={popupInnerRef} className={styles.popupInner}>
            <img src={currentConfig.img} className={styles.popupImage} alt={currentConfig.alt} />
  
            {/* klik for at lukke */}
            <button className={styles.closeHitArea} onClick={handleClosePopup} />
